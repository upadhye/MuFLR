...
double ynu(int ny, int alpha, int ell, int ik, const double *y){
  if(ny<0 || alpha<0 || alpha>=N_tau || ell<0 || ell>=N_mu || ik<0 || ik>=NK)
    return 0;
  if(ny==0) return ynu0(alpha,ell,ik,y);
  else if (ny==1) return ynu1(alpha,ell,ik,y);
...
int print_all_Pmono(double z, const double *w){
  ...
  for(int i=0; i<NK; i++){
    ...
    for(int t=0; t<N_tau; t++)
      printf("   %e %e %e",
	     sq(ynu0(t,0,i,w)), ynu0(t,0,i,w)*ynu1(t,0,i,w), sq(ynu1(t,0,i,w)));
    printf("\n");
    fflush(stdout);
  }
  return 0;
...
int der(double eta, const double *y, double *dy, void *par){
  ...
  double Xi[NK][2][2], fnc=C->f_nu_0/C->f_cb_0; //linear evolution matrix
...
#pragma omp parallel for schedule(dynamic)
  for(int i=0; i<NK; i++){
    ...
    double dnu = d_nu_mono(i,ze,y), dcb = ee*sqrt(Pcb0n(i,y));
    Xi[i][0][0] = 1.0;
    Xi[i][0][1] = -1.0;
    Xi[i][1][0] = -1.5 * OF_eta(N_tau,eta,*C) * (1.0 + fnc*dnu/dcb);
    Xi[i][1][1] = 2.0 + dlnHc;
...
#pragma omp parallel for schedule(dynamic)
  for(int i=0; i<NK; i++){
    ...
    for(int t=0; t<N_tau; t++){
      ...
      w[(2*t+1)*N_mu*NK + 0*NK + i] = dlnFt*ynu0(t,0,i,w) + Ft*ycb1l(i,w);
    }
